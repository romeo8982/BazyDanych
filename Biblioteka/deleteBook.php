...
if(isset($_SESSION['log'])&&($_SESSION['log']==true)&&!empty($_POST))
{
    ...
    <div class = "web_page" >
    <?php
    $_SESSION['idBookToDelete'] = $_POST['delete'];
	$idBook = $_SESSION['idBookToDelete'];
    if($sqlDeleteUser = $polaczenie->query("DELETE FROM book WHERE id = $idBook")===TRUE)
	{
		?>
		<h1>Książka została usunięta</h1>
		<script>
